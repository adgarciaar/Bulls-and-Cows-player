...
class BullsAndCows_proyecto( BullsAndCowsBasePlayer ):
    ...
    def __init__( self, n, a ):
        ...
        self.moviendoPicaEncontrada = False
        self.picaMoviendose = None
        self.posicionInicialPicaEncontrada = None
        self.posicionMovimientoPica = 0
        self.caracterCambiadoPorPica = None
    ...
    def guess( self, b, c ):
        ...
        if (self.moviendoPicaEncontrada == True):

            if( b > self.fijasIntentoAnterior ): #la pica se volvió fija

                nuevaFija = Tupla( self.posicionMovimientoPica, self.m_CurrentSolution[self.posicionMovimientoPica] )
                self.fijasDetectadas.append(nuevaFija)

                #guardar dónde hay una fija
                self.posicionesFijas[ self.posicionMovimientoPica ] = True
                self.posicionesValidas[ self.posicionMovimientoPica ] = True
            ...
            else: #continuar moviendo la pica

                #revertir cambio
                listCurrentSolution = list(self.m_CurrentSolution)
                listCurrentSolution[ self.posicionMovimientoPica ] = self.caracterCambiadoPorPica
                self.m_CurrentSolution = ''.join(listCurrentSolution)

                while ( self.posicionesFijas[self.posicionMovimientoPica] == True or self.posicionInicialPicaEncontrada == self.posicionMovimientoPica ):
                    self.posicionMovimientoPica = self.posicionMovimientoPica+1

                listCurrentSolution = list(self.m_CurrentSolution)
                self.caracterCambiadoPorPica = listCurrentSolution[ self.posicionMovimientoPica ]
                listCurrentSolution[ self.posicionMovimientoPica ] = self.picaMoviendose
                self.m_CurrentSolution = ''.join(listCurrentSolution)
            ...
            if(self.cambioFueRealizado == True):
                ...
                if( b !=  self.fijasIntentoAnterior or c != self.picasIntentoAnterior):
                    ...
                    if(b > self.fijasIntentoAnterior): #lo que se agregó es una fija
                        ...
                        self.posicionMovimientoPica = self.posicionDelCambio

                        while ( self.posicionesFijas[self.posicionMovimientoPica] == True or self.posicionInicialPicaEncontrada == self.posicionMovimientoPica ):
                            self.posicionMovimientoPica = self.posicionMovimientoPica+1

                        self.picaMoviendose = self.m_CurrentSolution[self.posicionDelCambio]
                        listCurrentSolution = list(self.m_CurrentSolution)
                        self.caracterCambiadoPorPica = listCurrentSolution[ self.posicionMovimientoPica ]
                        listCurrentSolution[ self.posicionMovimientoPica ] = self.picaMoviendose
                        self.m_CurrentSolution = ''.join(listCurrentSolution)

                        self.moviendoPicaEncontrada = True
